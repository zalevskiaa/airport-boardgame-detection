...
def carve_fields(model_path, input_images_dirpath, output_images_dirpath):
    ...
    for img_filename in images_filenames:
        ...
        result = detect_field(model, img)
        if result:
            x1, y1, x2, y2 = result
            carved_img = img[y1:y2, x1:x2]
            carved_img_filename = f'{img_filename[:-4]}-field{img_filename[-4:]}'
            carved_img_filepath = \
                os.path.join(output_images_dirpath, carved_img_filename)
            imwrite(carved_img_filepath, carved_img)


def main():
    carve_fields(model_path, input_images_dirpath, output_images_dirpath)
